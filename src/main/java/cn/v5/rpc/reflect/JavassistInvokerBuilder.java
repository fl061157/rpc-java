...
import cn.v5.rpc.reflect.ReflectionInvokerBuilder.ReflectionArgumentEntry;
import javassist.*;
...
public class JavassistInvokerBuilder extends InvokerBuilder {
        ...
        public void invoke(Object target, Request request) throws Exception {
            ...
            try {
                if (LOG.isDebugEnabled()){
                    LOG.debug("invoke method {}, params: {}", method.getName(), params);
                }
    ...
    public JavassistInvokerBuilder(MessagePack messagePack) {
        ...
        addClassLoader(AbstractInvoker.class.getClassLoader());
    ...
    private String buildInvoke0MethodBody(Method m, ReflectionArgumentEntry[] res) {
        ...
        for (int i = 0; i < paramTypes.length; ++i) {
            ...
            boolean isArray = paramTypes[i].isArray();
            if (isPrimType) { // ((Integer) $2[0]).intValue()
                buildString("((%s) $2[%d]).%s()",
                        new Object[]{toWrapperType(type).getName(), i, toPrimTypeMethod(type)});
            }else if(isArray){
                Class<?> arrayType = paramTypes[i].getComponentType();
                boolean arrayIsPrimType = arrayType.isPrimitive();
                if (arrayIsPrimType){
                    buildString("(%s[]) $2[%d]", new Object[]{toArrayTypeMethod(arrayType), i});
                }else {
                    buildString("(%s[]) $2[%d]", new Object[]{arrayType.getName(), i});
                }
            } else { // (Integer) $2[0]
    ...
    protected static String toArrayTypeMethod(Class<?> type) {
        if (type.equals(boolean.class)) {
            return "boolean";
        } else if (type.equals(byte.class)) {
            return "byte";
